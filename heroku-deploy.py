...
def init_deploy():
    shutil.rmtree(os.path.join(./, 'todo_web'))
...
def build_frontend():
    os.system('cd todo_frontend && npm run build')


def copy_build_folder():
    shutil.copytree(
        os.path.join(FILE_PATH, 'todo_frontend', 'build'),
        os.path.join(FILE_PATH, 'todo_web', 'build'))
    shutil.rmtree(os.path.join(FILE_PATH, 'todo_frontend', 'build'))
